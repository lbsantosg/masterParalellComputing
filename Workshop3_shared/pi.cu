...
/*
    for (int i=0; i < n_Blocks; i++) {
        int init_iter_sp = (n_sp / blocks) *  i;
        int end_iter_sp = (i == n_Blocks-1 ? n_sp : init_iter_sp + (n_sp/blocks)); 
...
*/
    ...
    img_blr<<<6,256>>>(d_input_img, d_output_img, width, height, channels, kernel_sz, total_super_pixels, sz_super_pixel, n_Blocks,  256);
